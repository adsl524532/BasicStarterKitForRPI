...
#define myLed_pin  0     // GPIO pin 17
#define myLed_pin2 2     // GPIO pin 27
...
//      digitalWrite(AF_LED,0); 		//Close back light
...
//	   lcdPuts(lcdHandle, szIP_address);
	   ...
	   lcdPuts(lcdHandle, szTime);
           ...
           getTemperature(&T, &T8);   	// DS18B20 takes about 1 sec to execute
           if (T8-T>2.0) {              // T8 radiator temp more then 2 deg higher
              digitalWrite(myLed_pin , HIGH);
           } else {
              digitalWrite(myLed_pin, LOW);
           }
           if (T8 > T8_prv+0.10) {         // Radiator temp. is rising
              T8_prv = T8;
              digitalWrite(myLed_pin2, HIGH);
           } else if (T8 < T8_prv-0.0) {
              T8_prv = T8;
              digitalWrite(myLed_pin2, LOW);
           }



           sprintf(szT , "DS18B20: T=%.3f", T);
           sprintf(szT8, "DS18B20: T=%.3f : %.4f", T8, T8 - T);   // show difference
           printf("%s\n",szT);
           printf("%s\n",szT8);
...
//	   lcdClear(lcdHandle); 	//Clear display
	   ...
	   char *szFilename2 = "/home/pi/Chris/Logs/Upload_Temperature.sh";
           ...
           if (fp2) {
              ...
              fprintf(fp2,"/usr/bin/curl \"https://api.thingspeak.com/update?api_key=6RJ7LMZ7V0JJSUK8");
              fprintf(fp2,"&field1=%.3f\"\n", T8);
              fclose(fp2);
           } else
	      printf("Cannot open file %s\n", szFilename2);
...
float getTemperature(float *T1, float *T2)   
{
   ...
   fclose(fd);
   *T1 =atof(szString2+29)/1000;  // temp is at pos 29
